...
\begin{tikzpicture}[>=latex']
	...
	\coordinate (origin) at (0,-4cm);
	\node [circle, draw, at=(origin)] (velocita) {$\mathbf{v}$};

	\node [rectangle,draw, text width=2cm, at=(velocita), xshift=2cm] (conoscenza) {Previous knowledge};
	\node [circle, draw, at=(velocita), shift=(90:1.3cm), color=red] (angolo) {$\psi$};

	\node [circle, draw, at=(velocita), shift=(270:1.3cm), color=blue] (magnitude) {$|\mathbf{v}|$};

	\node [circle, draw, at=(angolo), anchor=south east, shift=(135:1cm), color=red] (cos) {$\cos\theta$};
	\node [circle, draw, at=(angolo), anchor=south,shift=(90:1cm), color=red] (nabla) {$\nabla\mathbf{H}$};
	\node [circle, draw, at=(angolo), anchor=south west,shift=(45:1cm), color=red] (sin) {$\sin\theta$};

	\node [circle, draw, at=(magnitude), anchor=north,shift=(270:1cm), color=blue] (maggg) {$|\mathbf{H}|$};
